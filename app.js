...
async function fetchPoint(point) {
  ...
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${point.lat}&longitude=${point.lon}` +
    `&hourly=${vars}&timezone=Europe%2FCopenhagen&forecast_days=2&wind_speed_unit=ms`;
...
function scoreWeather(w) {
  ...
  if (w.wind >= 9.7) { score -= 33; reasons.push("meget vind"); }
  else if (w.wind >= 6.9) { score -= 21; reasons.push("frisk vind"); }
  else if (w.wind >= 5.0) { score -= 10; reasons.push("mærkbar vind"); }

  if (w.gust >= 15.3) { score -= 20; reasons.push("kraftige vindstød"); }
  else if (w.gust >= 11.1) { score -= 10; reasons.push("vindstød"); }
...
function renderWindow(id, w) {
  ...
  el.innerHTML =
    ...
    metric("Regnniveau", rain.text, rain.cls) +
...
function dominantWeather(trips) {
  ...
  const worstSnow = max(trips.map(x => x.snow));
  const worstWind = max(trips.map(x => x.wind));
  const worstGust = max(trips.map(x => x.gust));

  const rainSeverity = Math.max(worstRain / 1.5, max(trips.map(x=>x.pop)) / 75);
  const snowSeverity = worstSnow > 0 ? 2 + worstSnow : 0;
  const windSeverity = Math.max(worstWind / 7, worstGust / 11);

  if (snowSeverity >= rainSeverity && snowSeverity >= windSeverity && snowSeverity > 0.5) return "snow";
  if (rainSeverity >= windSeverity && rainSeverity > 0.5) return "rain";
  if (windSeverity > 0.5) return "wind";
  return "sun";
}

function setWeatherAnimation(type) {
  const el = document.getElementById("weatherAnimation");
  el.className = `weather-animation ${type}`;
}
...
function openClothes(key) {
  ...
  document.getElementById("clothesWeather").textContent =
    `Føles som ca. ${Math.round(all.feels)}°, vind op til ${all.wind.toFixed(1)} m/s og ${Math.round(all.pop)}% regnrisiko i jeres cykeltider.`;
...
async function loadWeather() {
  ...
  try {
    ...
    setWeatherAnimation(dominantWeather(allTrips));
    ...
    details += worstRain >= .2
      ? ` Op til ${worstRain.toFixed(1)} mm nedbør i en af perioderne.`
      : " Ingen nævneværdig regn i cykeltiderne.";
